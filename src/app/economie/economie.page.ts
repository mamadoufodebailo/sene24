...
@Component({
  ...
  templateUrl: './economie.page.html',
  styleUrls: ['./economie.page.scss'],
})
export class EconomiePage implements OnInit {
  donnees: any;
  slogan: string;
  totalPages: number;
  page: number = 1;
  ...
  onSearch(){
    this.api.getActualiteByCategorie('economie',this.page).subscribe(data => {
      this.donnees = data;
      ...
      this.storage.set('economies',this.actualites);
      },error => {
      this.storage.get('economies').then(data => {
        this.donnees = data;
