...
export class InternationalPage implements OnInit {
  ...
  onSearch(){
    this.api.getActualiteByCategorie('international',this.page).subscribe(data => {
      this.donnees = data;
      ...
      this.storage.set('internationaux',this.actualites);
      },error => {
      this.storage.get('internationaux').then(data => {
        this.donnees = data;
